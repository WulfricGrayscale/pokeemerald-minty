...
#define tShinyOdds data[7]
#define tGiftRerolls data[8]
...
// Page 2
#define YPOS_SHINYODDS    (MENUITEM_SHINYODDS * 16)
#define YPOS_GIFTREROLLS  (MENUITEM_GIFTREROLLS * 16)
...
static u8 GiftRerolls_ProcessInput (u8 selection);
static void GiftRerolls_DrawChoices (u8 selection);
...
{
    [MENUITEM_SHINYODDS]   = gText_BaseOdds,
    [MENUITEM_GIFTREROLLS] = gText_ExtraRerolls,
...
static void ReadAllCurrentSettings(u8 taskId)
{
    ...
    gTasks[taskId].tShinyOdds = FlagGet(FLAG_CLASSIC_SHINY_ODDS);
    gTasks[taskId].tGiftRerolls = FlagGet(FLAG_GIFT_REROLL);
}
...
static void DrawOptionsPg2(u8 taskId)
{
    ...
    GiftRerolls_DrawChoices(gTasks[taskId].tGiftRerolls);
    HighlightOptionMenuItem(gTasks[taskId].tMenuSelection);
    CopyWindowToVram(WIN_OPTIONS, COPYWIN_FULL);
}
    ...
    else
    {
        ...
        case MENUITEM_SHINYODDS:
            ...
            if (previousOption != gTasks[taskId].tShinyOdds)
                ShinyOdds_DrawChoices(gTasks[taskId].tShinyOdds);
            break;
        case MENUITEM_GIFTREROLLS:
            previousOption = gTasks[taskId].tGiftRerolls;
            gTasks[taskId].tGiftRerolls = GiftRerolls_ProcessInput(gTasks[taskId].tGiftRerolls);

            if (previousOption != gTasks[taskId].tGiftRerolls)
                GiftRerolls_DrawChoices(gTasks[taskId].tGiftRerolls);
            break;
        default:
            return;
...
static void Task_OptionMenuSave(u8 taskId)
{
    ...
    gTasks[taskId].tShinyOdds == 0 ? FlagClear(FLAG_CLASSIC_SHINY_ODDS) : FlagSet(FLAG_CLASSIC_SHINY_ODDS);
    gTasks[taskId].tGiftRerolls == 0 ? FlagClear(FLAG_GIFT_REROLL) : FlagSet (FLAG_GIFT_REROLL);
...
static void ShinyOdds_DrawChoices(u8 selection)
{
    ...
    styles[1] = 0;
    styles[selection] = 1;
    DrawOptionMenuChoice(gText_ModernOdds, 104, YPOS_SHINYODDS, styles[0]);
    DrawOptionMenuChoice(gText_ClassicOdds, GetStringRightAlignXOffset(FONT_NORMAL, gText_ClassicOdds, 198), YPOS_SHINYODDS, styles[1]);
}
...
static void GiftRerolls_DrawChoices(u8 selection)
{
    ...
    styles[1] = 0;
    styles[selection] = 1;
    DrawOptionMenuChoice(gText_NoRerolls, 104, YPOS_GIFTREROLLS, styles[0]);
    DrawOptionMenuChoice(gText_GiftRerolls, GetStringRightAlignXOffset(FONT_NORMAL, gText_GiftRerolls, 198), YPOS_GIFTREROLLS, styles[1]);
}
